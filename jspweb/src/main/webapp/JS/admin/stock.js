...
function getcategory() {
	$.ajax({
		...
		type : "get",
		success : function(re) {
			let categorylist = JSON.parse(re)
...
function getproduct(pcno) {
	$.ajax({
		...
		success :  function (re) {
			...
			let html = '<option>상품 선택</option>'
			productlist.forEach(p => {
				if(p.pcno == pcno) { // 해당 제품의 카테고리 번호와 카테고리 번호와 같으면
