...
public class ProductDao extends Dao {
	...
	public boolean setProduct (ProductDto dto) {
		...
		String sql = "insert into product( pno, pname , pcomment , pprice , pdiscount , pactive , pimg , pcno) values(?,?,?,?,?,?,?,?)";
		try {
			ps = con.prepareStatement(sql);
			ps.setInt( 1 , dto.getPno());	ps.setString( 2 , dto.getPname());
			ps.setString( 3 , dto.getPcomment());	ps.setInt( 4 , dto.getPprice());
			ps.setFloat( 5 , dto.getPdiscount());	ps.setByte( 6, dto.getPactive());
			ps.setString( 7 , dto.getPimg());		ps.setInt( 8 , dto.getPcno());
			ps.executeUpdate(); return true;
	...
	public ArrayList< ProductDto > getProductlist(String option){
		...
		if(option.equals("all")) {
			sql = "select * from product"; // 1. 조건없는 모든 제품 출력
			try {
				ps = con.prepareStatement(sql);
				rs = ps.executeQuery();
				while( rs.next() ) {
